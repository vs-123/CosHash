...
void derive_params(const std::vector<uint8_t>& block, uint32_t &a, uint32_t &b, uint32_t &c)
{
  ...
  a = (block.size() >= 4) ? 
        (block[0] << 24 | block[1] << 16 | block[2] << 8 | block[3]) : 0x1f1f1f1f;
  b = (block.size() >= 8) ? 
        (block[4] << 24 | block[5] << 16 | block[6] << 8 | block[7]) : 0x3c3c3c3c;
  c = (block.size() >= 12) ? 
        (block[8] << 24 | block[9] << 16 | block[10] << 8 | block[11]) : 0x7a7a7a7a;
}
...
std::vector<uint8_t> cos_hash(const std::vector<uint8_t>& input)
{
  ...
  for (size_t b = 0; b < num_blocks; ++b) {
    std::vector<uint8_t> current_block(
      paddedInput.begin() + b * BLOCK_SIZE,
      paddedInput.begin() + (b + 1) * BLOCK_SIZE
    );
    ...
    for (int i = 0; i < STATE_SIZE; ++i) {
      ...
      if (offset + 4 <= current_block.size()) {
        segment = (current_block[offset] << 24) |
                  (current_block[offset + 1] << 16) |
                  (current_block[offset + 2] << 8) |
                  (current_block[offset + 3]);
      } else {
